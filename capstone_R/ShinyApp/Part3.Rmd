---
title: "part2: Advanced Predictive Model and Eval"
author: "jameel"
date: "2024-05-28"
...
This report outlines the process of building and evaluating an advanced predictive model using n-grams, RNNs, and other techniques. The goal is to improve accuracy while reducing computational runtime and model complexity.
...
## Pruning and Smoothing N-gram Models
```{r smooth-n-gram model}
create_ngram <- function(tokens, n) {
  ngrams <- unlist(lapply(seq_along(tokens), function(i) {
    if (i <= length(tokens) - n + 1) {
      paste(tokens[i:(i + n - 1)], collapse = " ")
...
test_data <- sample_file(file_path, 0.001)  # Smaller sample size
...
```{r RNN model}
# Function to build a simple RNN model using Keras
build_rnn_model <- function(input_dim, output_dim) {
  model <- keras_model_sequential() %>%
    layer_embedding(input_dim = input_dim, output_dim = output_dim, input_length = 50) %>%
...
evaluate_rnn_model <- function(model, x_test, y_test) {
  scores <- model %>% evaluate(x_test, y_test)
  print(paste("Test Loss:", scores$loss))
  print(paste("Test Accuracy:", scores$acc))
}
